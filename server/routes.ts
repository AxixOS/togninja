...
async function generateModernInvoicePDF(invoice: any, client: any): Promise<Buffer> {
  ...
  // THE STUDIO'S OWN LOGO, and a header photograph of the client.
  //
  // `logoBase64` above is a hardcoded PNG of the ORIGIN studio's logo, drawn on every
  // invoice on every instance built from this image — another business's brand at the top
  // of a document asking somebody for money. It is now the last resort only, behind the
  // studio's own logo_url and behind their name in text.
  //
  // Both images are fetched HERE because jsPDF is synchronous once drawing begins.
  const { fetchHeaderImage, resolveDocumentHeader } = await import('./lib/documentHeader');
  const toDataUri = (b: Buffer | null, mime = 'image/jpeg') =>
    (b ? `data:${mime};base64,${b.toString('base64')}` : null);

  const studioLogo = toDataUri(await fetchHeaderImage(brand.logoUrl), 'image/png');
  ...
  // A photograph across the top, when there is one to show.
  if (headerPhoto) {
    try {
      doc.addImage(headerPhoto, 'JPEG', 0, 0, pageWidth, 34);
    } catch { /* an unreadable header must not cost the invoice */ }
  }
  const headerOffset = headerPhoto ? 34 : 0;

  try {
    if (!studioLogo) throw new Error('no studio logo');
    doc.addImage(studioLogo, 'PNG', 15, headerOffset + 5, 45, 15);
  } catch (error) {
    // The studio's NAME, not the other studio's picture.
    doc.setFillColor(147, 51, 234);
    doc.rect(0, headerOffset, pageWidth, 25, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text((studioConfig.studioName || '').toUpperCase(), 20, headerOffset + 17);
  }

  yPosition = 30 + headerOffset;
