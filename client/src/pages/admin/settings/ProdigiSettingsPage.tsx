...
import { Printer, Save, AlertCircle, CheckCircle, FlaskConical, ArrowRight } from 'lucide-react';

/**
 * Prodigi print-fulfilment settings: the studio's own Prodigi credential, and nothing
 * else. The print CATALOGUE used to be bolted to the bottom of this page, which meant the
 * list a studio sells from was filed under API keys and was reachable only after the key
 * form. It lives at /admin/print-products now, with its own sidebar entry; the link below
 * is the only thing left here about it.
 *
 * Each studio brings its OWN Prodigi API key
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 max-w-2xl text-sm text-blue-800 space-y-1">
          <p><strong>How it works:</strong> build your list of print products first, then place orders from an invoice — your client pays, and the order goes to Prodigi on this key, on your account, under your name.</p>
        ...
        <div className="bg-white rounded-lg shadow p-6 max-w-2xl">
          <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
            <Printer size={18} className="text-purple-600" /> Your print products
          </h2>
          <p className="text-sm text-gray-600 mt-1">
            The sizes you sell, what each one costs you and what you charge for it, live on
            their own page now — you do not need this key to look at them or to price them.
          </p>
          <Link
            to="/admin/print-products"
            className="mt-3 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-purple-600 text-white hover:bg-purple-700 text-sm"
          >
            Open Print Products <ArrowRight size={16} />
          </Link>
        </div>
      </div>
    </AdminLayout>
